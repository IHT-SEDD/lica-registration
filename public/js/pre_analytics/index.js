...
var onSelectTransaction = function (selectedData) {
  ...
  // check status interfacing
  if(selectedData.status_interfacing == 1){
    $("#btn-reset-analyzer").prop('disabled', false);
    $("#btn-reset-analyzer").data('transaction-id', transactionId);
  }
...
let resetStatusInterfacing = () => {
  $("#btn-reset-analyzer").on('click', function() {
    const transactionId = $(this).data('transaction-id');
    $.ajax({
      url: baseUrl('pre-analytics/reset-analyzer/' + transactionId),
      type: 'PUT',
      success: function(res) {
        toastr.success(res.message); 
      },
