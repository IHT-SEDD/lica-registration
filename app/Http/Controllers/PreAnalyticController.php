...
class PreAnalyticController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            if (!$request->patient_id) {
                ...
                Log::channel('preanalytic_activity')->info('PreAnalyticController@create => Pasien baru sukses dibuat', ['data' => $new_patient]);
            ...
            if ($room->auto_checkin || $room->auto_draw) {
                ...
                DB::transaction(function () use ($prefixDate,  &$requestData) {

                    $noLab = 0;
                    $NolabExists = DB::table('transactions')
                        ->where('no_lab', 'like', $prefixDate . '%')
                        ->orderBy('no_lab', 'desc')
                        ->first();
                
                    if ($NolabExists) {
                        $noLab +=  $NolabExists->no_lab;
                    }else{
                        $trxId = str_pad(1, 3, '0', STR_PAD_LEFT);
                        $noLab = $prefixDate . $trxId;
                    }
                                        
                    $requestData['no_lab'] = $noLab;
                });
            ...
            Log::channel('preanalytic_activity')->info('PreAnalyticController@create => Data Pre Analytic telah dibuat', ['data' => $requestData]);
    ...
    public function deleteTransaction($id)
    {
        try {
            ...
            Log::channel('preanalytic_activity')->info("PreAnalyticController@deleteTransaction => Data Transaksi {$id} berhasil di Hapus", [
                'data' => $data
            ]);
    ...
    public function checkIn($isManual = false, Request $request)
    {
        try {
            ...
            if ($isManual) {
                ...
                DB::transaction(function () use ($prefixDate, $transaction) {
                    // $countExistingData = DB::table('transactions')
                    //     ->where('no_lab', 'like', $prefixDate . '%')
                    //     ->lockForUpdate()
                    //     ->count();
                    $noLab = 0;
            ...
            Log::channel('preanalytic_activity')->info("PreAnalyticController@checkIn => Data Transaksi {$transaction->no_lab} berhasil di Check In", [
                'data' => $transaction
            ]);            
    ...
    public function resetAnalyzer($transactionId)
    {
    try{

        \App\Transaction::whereId($transactionId)->update(['status_interfacing' => 0]);

        return response()->json(['message' => 'Successfully update status interfacing', 'status' => 200]);
