...
class AnalyticController extends Controller
{
    ...
    public function datatable($startDate = null, $endDate = null, $group_id = null)
    {
        ...
        return DataTables::of($model)
            ->addIndexColumn()
            ->addColumn('wait_time', function($data){
                $transactionId = $data->id;
                $cito = DB::table('transactions')
                        ->select('cito')->where('id', $transactionId)->first();

                        // Ambil group_id dan test_id sebagai array
                $group_ids = DB::table('transaction_tests')
                            ->where('transaction_id', $transactionId)
                            ->pluck('group_id')
                            ->toArray();
                        
                $test_ids = DB::table('transaction_tests')
                            ->where('transaction_id', $transactionId)
                            ->pluck('test_id')
                            ->toArray();
                        
                        // Inisialisasi nilai wait_time
                $wait_time = null;
                        
                if ($cito->cito === null) {
                    if (in_array(16, $group_ids) || array_intersect([741, 950, 1270, 1271, 132], $test_ids)) {
                        $wait_time = 0;
                    } elseif (array_intersect([712, 740, 590], $test_ids) && in_array(1, $group_ids)) {
                        $wait_time = 120;
                    } else {
                        // Ambil target_tat
                        $wait_time = DB::table('transaction_tests')
                                    ->leftJoin('groups', 'transaction_tests.group_id', '=', 'groups.id')
                                    ->where('transaction_tests.transaction_id', $transactionId)
                                    ->orderBy('groups.target_tat', 'DESC')
                                    ->pluck('groups.target_tat')
                                    ->toArray();
                    }
                } else {
                    // Ambil target_tat_cito
                    $wait_time = DB::table('transaction_tests')
                                ->leftJoin('groups', 'transaction_tests.group_id', '=', 'groups.id')
                                ->where('transaction_tests.transaction_id', $transactionId)
                                ->orderBy('groups.target_tat_cito', 'DESC')
                                ->pluck('groups.target_tat_cito')
                                ->toArray();
                    }
                        
                        // Hitung max_wait_time
                        if ($wait_time) {
                            if ($wait_time === 120) {
                                $max_wait_time = 120;
                            } else {
                                $wait_time_array = array_map('intval', is_array($wait_time) ? $wait_time : explode(',', $wait_time));
                                $max_wait_time = max($wait_time_array);
                            }
                        } else {
                            $max_wait_time = 0;
                        }
    ...
    {
        $model_query = \App\TransactionTest::selectRaw('transaction_tests.*, transaction_tests.id as tt_id, results.result as res_label, packages.name as package_name, transactions.checkin_time as checkin_time, transactions.insurance_id as insurance_id, groups.target_tat as tat_target, groups.target_tat_cito as tat_target_cito,  transactions.checkin_time as checkin_time, transaction_tests.transaction_id, transactions.cito as cito')
        ...
        // $model_tat_target = \App\TransactionTest::selectRaw('groups.target_tat as tat_target, transactions.checkin_time as checkin_time')
        //     ->leftJoin('groups', 'groups.id', 'transaction_tests.group_id')
        //     ->leftJoin('transactions', 'transactions.id', 'transaction_tests.transaction_id')
        //     ->where('transaction_id', $transactionId)
        //     ->groupBy('transaction_tests.group_id');
        // $tat_target = $model_tat_target->get();


        $transaction = \App\Transaction::findOrFail($transactionId);
        // $arrayMerge = collect($model)->merge($tat_target);
    ...
    public function updateResultNumber($transactionTestId, Request $request)
    {
        try {
            ...
            preg_match('/([\D]*)([\d.]*)/', $result_temp, $matches);
            $result_symbol = $matches[1];
            $result_temp = trim($matches[2]);
            $currentResult = trim($matches[2]);
            ...
            if($result_symbol != '' && $result_symbol != null){
            ...
            $status = $this->checkResultStatus($patient, $range, $currentResult);
    ...
    private function checkResultStatus($patient, $range, $currentResult)
    {

        $status = '';
        if ($patient->gender == 'M') {
            if ($currentResult >= $range->min_male_ref && $currentResult <= $range->max_male_ref) {
                $status = 'normal';
            } else if ($currentResult < $range->min_crit_male || $currentResult > $range->max_crit_male) {
                $status = 'critical';
            } else if ($currentResult < $range->min_male_ref) {
                $status = 'low';
            } else if ($currentResult > $range->max_male_ref) {
                $status = 'high';
            }
        } else {
            if ($currentResult >= $range->min_female_ref && $currentResult <= $range->max_female_ref) {
                $status = 'normal';
            } else if ($currentResult < $range->min_crit_female || $currentResult > $range->max_crit_male) {
                $status = 'critical';
            } else if ($currentResult < $range->min_female_ref) {
                $status = 'low';
            } else if ($currentResult > $range->max_female_ref) {
                $status = 'high';
            }
        }
    ...
    public function checkValidatedTestGroup($transactionId, $groupId = null)
    {
        $query_all_group_test = DB::table('transaction_tests')->selectRaw('count(group_id) as total, groups.name as group_name')
        ->leftJoin('groups', 'groups.id', '=', 'transaction_tests.group_id')
        ->where('transaction_tests.transaction_id', $transactionId)
        ->groupBy('group_id')
        ->get();

        $query_all_group_test_valid = DB::table('transaction_tests')->selectRaw('count(group_id) as total, groups.name as group_name')
        ->leftJoin('groups', 'groups.id', '=', 'transaction_tests.group_id')
        ->where('transaction_tests.transaction_id', $transactionId)
        ->where('validate', 1)
        ->groupBy('group_id')
        ...
        foreach ($query_all_group_test as $value) {
            $validated_per_group[$value->group_name] = 0;

            foreach ($query_all_group_test_valid as $value2) {
                if ($value2->group_name == $value->group_name) {
                    if ($value2->total == $value->total) {
                        $validated_per_group[$value->group_name] = 1;
                        break;
                    }
                }
            }
        ...
        $query_test = DB::table('transaction_tests')->selectRaw('transaction_tests.transaction_id, GROUP_CONCAT(tests.id SEPARATOR ",") as test_ids, GROUP_CONCAT(tests.name SEPARATOR ",") as test_names, groups.name as group_name')
        ->leftJoin('tests', 'tests.id', '=', 'transaction_tests.test_id')
        ->leftJoin('groups', 'groups.id', '=', 'transaction_tests.group_id')
        ->where('transaction_tests.transaction_id', $transactionId)
        ->where('transaction_tests.validate', 1)
        ->groupBy('transaction_tests.transaction_id', 'groups.name');
        $testData = $query_test->get();

        return response()->json(['data' => $validated_per_group]);
    }
}
