...
foreach ($table as $key => $value) {
    ...
    $groupId = $value->test->group->id;

    $tat_target = $value->test->group->target_tat;
    if ($currentGroupName != $groupName) {
        $currentGroupoName = $groupName;
    }
    $check_in = $value->checkin_time; 
    $testId = $value->id;
    $transactionId = $value->transaction_id;
    $timerId = "timer-" . $transactionId."-".$groupId; // Unique timer ID
    $bgId = "bg-" . $transactionId."-".$groupId; // Unique progress bar ID
    $txtId = "txt-" . $transactionId."-".$groupId; // Unique text display ID
        ...
        <tr>
            <td colspan="8">
                <h5 class="group-name" data-group-id="{{$value->group_id}}">{{$groupName}}
                    ...
                    <!-- <span class="tat-target"  
                    data-check-in="{{ $check_in }}" 
                    data-tat-target="{{ $tat_target }}"
                    data-timer-id="{{ $timerId }}"
                    data-bg-id="{{ $bgId }}"
                    data-txt-id="{{ $txtId }}"> {{ $tat_target }} </span>
                    <div id="{{ $timerId }}" class="bg-progress-bar">
                        <div id="{{ $bgId }}" style="width:0%; height:20px; background-color:green;"></div>
                        <span id="{{ $txtId }}"></span> -->
                    </div>
                </h5>
            </td>
...
<script>
    ...
    var transactionId = $('.tat-target').data('transaction-id')
    var groupId = $('.tat-target').data('group')
    var checkValidatedTestGroup = function (transactionId, groupId) {
        $.ajax({
            url: baseUrl('analytics/check-validated-test-group/' + transactionId + '/' + groupId),
            type: 'GET',
            success: function(res) {
                console.log(res);
            }
    ...
    //     var transactionId = $('.tat-target').data('transaction-id')
    //     var groupId = $('.tat-target').data('group-id')
    //     checkValidatedTestGroup(transactionId, groupId)
    //     $('.tat-target').each(function () {
    //         var checkIn = $(this).data('check-in');
    //         var targetTat = $(this).data('tat-target');
    //         var timerId = $(this).data('timer-id');
    //         var bgId = $(this).data('bg-id');
    //         var txtId = $(this).data('txt-id');
    //         var validate = $(this).data('validate');
    //         var createdTime = Date.parse(checkIn);
    //         var waitTime = targetTat * 60 * 1000;
    //         var endTime = createdTime + waitTime;

    //         if (isNaN(createdTime)) {
    //             $(`#${bgId}`).css({ "background-color": "transparent", "width": "100%" }).text("Belum dimulai");
    //             return;
    //         } 
    //         if (targetTat == null || targetTat == 0) {
    //             $(`#${bgId}`).css({ "background-color": "transparent", "width": "100%" }).text("Tidak ada waktu tunggu");
    //             return; 
    //         }
    //         if (validate == 1) {
    //             $(`#${bgId}`).css({ "background-color": "green", "width": "100%" }).text("selesai");
    //             return; 
    //         } else {
    //             $(`#${timerId}`).css({ "width": "100%", "background-color": "#ddd", "height": "20px"});
    ...
    //                         $(`#${bgId}`).css({ "background-color": "orange", "width": `${remainingPercentage}%` }); //Waktu tunggu tersisa 40%
    //                     } else {
    //                         $(`#${bgId}`).css({ "width": `${remainingPercentage}%` });
    //                     }
    //                     $(`#${txtId}`).text(`${hours}:${minutes}:${seconds}`);     
    //                 }
    //             }, 1000);
    //         }
    //     });
